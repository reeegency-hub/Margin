...
function buildNavGroups(isAdmin: boolean): NavGroupConfig[] {
  const main: NavGroupConfig = {
    id: "main",
    items: NAV_SECTIONS.map((section) => {
      const children = (section.children ?? [])
        // Pas de doublon « Vue d’ensemble » ; garder les vrais sous-écrans
        .filter((child) => {
          const label = child.label.toLowerCase();
          if (label.includes("vue d’ensemble") || label.includes("vue d'ensemble")) {
            return false;
          }
          return true;
        })
...
function ShellInner({
  ...
  const navGroups = useMemo(() => buildNavGroups(Boolean(isAdmin)), [isAdmin]);
  ...
  const topbarTitle =
    ...
    NAV_SECTIONS.find((s) => pathname === s.href || pathname.startsWith(`${s.href}/`))
      ?.label;

  const bottomNavClass = forceMobile
    ? "block"
    : isHome
      ? "lg:hidden"
      : "md:hidden";
    ...
    >
      ...
      >
        ...
        {showFullscreenGuide ? (
          ...
          <>
            {/* Un seul owner UX : dock FirstHour → coach page (handoff) ; tours coupés */}
